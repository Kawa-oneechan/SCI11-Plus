...
AvdPoint* getpath(AvdPoint *A, AvdPoint *B, polygon *polylist, int opt)
{
	...
	for (i = 0; polylist[i].n != 0; ++i)
	{
		if ((polylist[i].type >= MTP) || ((polylist[i].type == TAP) && (opt == FALSE)))
		{
			delPoly(polylist, i);
			--i;
		}
	...
	for (i = 0; polylist[i].n != 0; ++i)
	{
		if (ptIntr(A, polylist[i].polyPoints, polylist[i].n) || ((polylist[i].type == CAP) && (nearPoint(A, polylist[i].polyPoints, polylist[i].n, &I2, TRUE) < 2)))
		{
...
static long near nearPoint(AvdPoint *P, AvdPoint *Poly, int n, AvdPoint *R, int Edge)
{
	...
	for (i = 0; i < n; ++i)
	{
		...
		if (i == n-1)
			...
			B = Poly[i + 1];
		if (Edge && ((A.x == B.x) && ((A.x == 0) || (A.x == picWindPoly[1].x)) || (A.y == B.y) && ((A.y == 0) || (A.y == picWindPoly[2].y))))
		{
...
*/
			{
				//Distance P-R is: ABS{[(B-A)^(0, 0, 1)]#(A-P)/|B-A|}
				...
				if (dot2 < d)
				{
					...
					R->x = P->x + (int)((dot3 + sign(dot3) * (dot2 - 1)) / dot2);
					dot1 = dot1 * (long)tmp4.y;
					R->y = P->y + (int)((dot1 + sign(dot1) * (dot2 - 1)) / dot2);
				}
			}
			else
			{
...
static int near testClockwise(AvdPoint *Points, int n)
{
	...
	if (delta < -180) delta += 360;
	angle += delta;
	//at this point if previous nine lines are compiled then angle = +-360
	return(angle);
}


//returns heading from (x1, y1) to (x2, y2) in degrees (always in the range 0-359)
static int near ATan(int x1, int y1, int x2, int y2)
{
	int major;
		...
		for (i = 0; polylist[i].n != 0; ++i)
		{
			...
			//this polygon is an obstruction, create path node
			{
				...
				else
				{
					...
					{
						if (((node->I1.x != A->x) && (abs(newNode->I1.x - A->x) < abs(node->I1.x - A->x))) || ((node->I1.y != A->y) && (abs(newNode->I1.y - A->y) < abs(node->I1.y - A->y))))
						{
...
static int near PatchNode(polyPatch *Patch, int theNode, AvdPoint *thePoly)
{
	...
	if (Patch->P_j < Patch->P_i)
		if ((Patch->P_i < theNode) || (theNode <= Patch->P_j))
			return (TRUE);
	if (Patch->P_i == Patch->P_j)
		if (distEstimate(&Patch->P_U, &thePoly[Patch->P_i], &offScreen) > distEstimate(&Patch->Q_U, &thePoly[Patch->P_i], &offScreen))
			return (TRUE);
	return (FALSE);
}
...
static int near polypath(AvdPoint *A, AvdPoint *B, AvdPoint *P, int n, AvdPoint *I1, AvdPoint *I2, int *nodeI1, int *nodeI2)
{
	...
	if (intpoly(A, B, P, n, I1, I2, nodeI1, nodeI2) == 0)
		return(0);
	if (*nodeI1 == *nodeI2)
		return(0);
...
static int near lineOnScreenEdge(AvdPoint *p1, AvdPoint *p2)
{
	if ((p1->x == p2->x) && (p1->y != p2->y) && ((p1->x == 0) || (p1->x == picWindPoly[1].x)))
		return(1);
	if ((p1->y == p2->y) && (p1->x != p2->x) && ((p1->y == 0) || (p1->y == picWindPoly[2].y)))
		return(1);
	return(0);
}
