...
// > RunMovie had signed/unsigned mismatches.
// > LoadFrame had segment loss -- CAN'T FIX
...
{
	unsigned int i; //KAWA WAS HERE to make this unsigned.
	int done;
	int lastPage;
	unsigned celCount;
	...
	//KAWA WAS HERE to set these to unsigned.
	CopyPage(3 * 0x4000u, 0);
	ShowPage(3 * 0x4000u); //point to unused page
...
void LoadFrame(char page, int handle, Handle bufferHandle)
{
	...
	count = ReadDos(handle, (byte far*)&theCel, headerSize);
	if(count != headerSize)
	{
...
void LoadMoviePalette(int handle)
{
	...
	result = ReadDos(handle, (byte far*)&length, 4);
	if(result != 4)
	{
...
unsigned LoadCelCount(int handle)
{
	...
	result = ReadDos(handle, (byte far*)&celCount, 2);
	if(result != 2)
	{
