...
// > Signed/unsigned mismatches in FindResEntry and InitPatches, casts added to fix.
	...
	switch (resType)
	{
		...
		default:
			...
			if (scan = FindResEntry(resType, resId))
			{
				...
				//Although loaded, this resource MAY be in alternate memory. If this is the case it must be moved to main RAM
				if ((*scan)->altResMem)
				{
...
void ResUnLoad(byte resType, uword resId)
{
	...
	if (resId != ALL_IDS)
	{
		...
		{
			if ((*scan)->altResMem)
				AltResMemFree(scan); //free the data from standard memory
	...
	{
		for (scan = (LoadLink far**)Native(FFirstNode(&loadList)); scan; scan = tmp)
		{
			tmp = (LoadLink far**)Native(FNextNode(Pseudo(scan)));
			if ((*scan)->type == resType)
				ResUnLoad(resType, (*scan)->id);
		}
	}
}
...
//Return a Handle in concert with the Resource Manager purge resource list if necessary
Handle GetResHandle(uword size)
{
	Handle hndl;
...
void SetResCursor(ResCursor cursor)
{
	...
	if ((cursor == ArrowCursor && resourceCursorType) || (cursor == DiskCursor && resourceCursorType & RESCSR_DISKCHANGE) || (cursor == ARMReadCursor || cursor == ARMWriteCursor) && (resourceCursorType & RESCSR_ARMCHANGE))
	{
	...
	if ((cursor == DiskCursor && resourceCursorType & RESCSR_DISKMOVE) || (cursor == ARMReadCursor || cursor == ARMWriteCursor) && resourceCursorType & RESCSR_ARMMOVE)
	{
		RPoint mousePoint;
	...
	while (patchDir[dirNum])
	{
		...
		for (resType = RES_BASE; resType < RES_BASE + NRESTYPES; resType++)
		{
			...
			if (firstfile(filename,0,&fileinfo))
			{
				do
				{
					if (strchr(fileinfo.name,'.') && fileinfo.name[0] >= '0' && fileinfo.name[0] <= '9')
					{
						npatches++;
					}
			...
			if (firstfile(filename,0,&fileinfo))
			{
				do
				{
					if (strchr(fileinfo.name,'.') && fileinfo.name[0] >= '0' && fileinfo.name[0] <= '9')
					{
						...
						for (tEntry = (ResPatchEntry far*)*patches; tEntry != entry; tEntry++)
						{
							if (tEntry->resType == (byte)resType && tEntry->resId == (uword)resId) //KAWA WAS HERE
							{
								--npatches;
								break;
							}
						...
						if (tEntry == entry)
						{
							entry->patchDir = (byte)dirNum;
							entry->resType = (byte)resType;
							entry->resId = resId;
							++entry;
						}
					}
