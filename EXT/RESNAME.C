...
global char* ResNameMake(int resType, uint resId)
{
...
global char* ResNameMakeWildCard(int resType)
{
...
global char* ResName(int resType)
{
...
global char* addSlash(strptr dir)
{
	int len;
	char ch;

	if ((len = strlen(dir)) && (ch = dir[len - 1]) != '\\' && ch != '/' && ch != ':')
		strcat(dir, "\\");
	return dir;
}
