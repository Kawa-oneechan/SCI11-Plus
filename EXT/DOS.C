...
global strptr CleanDir(strptr dir)
{
	strptr dp;
...
If only path argument
	See if there is enough free space on the disk in 'path' to save a game.
else
...
global strptr GetDevice(strptr path, strptr device)
{
	if (*(path+1) != ':')
		GetCurDevice(device);
	else
...
global strptr GetCurDevice(strptr device)
{
	char path[65];
	getcwd(path);
...
global void GetExecDir(strptr path)
{
#ifdef DEBUG
	int i;
	char far *cmdPath;
