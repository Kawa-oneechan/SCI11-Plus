...
// > doScale was called twice with (word)NULL for Handle* unders.
// > Added kawaHax support.
...
static void	near SortCast(Obj* cast[], word casty[], word castpri[], int size);
static void	near ReDoStopped(Obj* cast[], byte castShow[], int size);
...
global void Animate(List* cast, word doit)
{
	AniObj *dup;
	register Obj *him;
	...
	word scale = 0, newXDim, newYDim, oldXDim, oldYDim, newXOff, newYOff, oldXOff, oldYOff;
	...
	for (it = FirstNode(cast); it; it = NextNode(it))
	{
		...
		else
		{
			...
			if (signal & STOPUPD || (signal & VIEWADDED))
			{
				++showAll;
			}
			signal &= ~FORCEUPD;
	...
	for (i = 0; i < castSize; i++)
	{
		...
		if (!(signal & (VIEWADDED | NOUPDATE | HIDE)))
		{
			...
			if((scale & VM_SCALABLE))
			{
				...
				IndexedProp(him, actUB) = Pseudo(SaveBits((RRect*)IndexedPropAddr(him, actNS), PMAP | VMAP | CMAP));
	...
	for (i = 0; i < castSize; i++)
	{
		...
		if ((castShow[i]) || (!(signal & HIDDEN) && (!(signal & NOUPDATE) || showAll)))
		{
			...
			if (CSectRect((RRect*)IndexedPropAddr(him, actLS), (RRect*)IndexedPropAddr(him, actNS), &ur))
			{
				//show the union
				RUnionRect((RRect*)IndexedPropAddr(him, actLS), (RRect*)IndexedPropAddr(him, actNS), &ur);
				ShowBits(&ur, showMap);
			}
			else
			{
			...
			RCopyRect((RRect*)IndexedPropAddr(him, actNS), (RRect*)IndexedPropAddr(him, actLS));
	...
	for (i = castSize - 1; i >=0; i--)
	{
		...
		if (signal & delObj) InvokeMethod(him, s_delete, 0);
	}
...
static void near doScale(Obj* vId, Handle theView, word lnum, word cnum, Handle* unders, word theScale, word saveUnders)
{
	word theScaleX, theScaleY, oldXDim, oldYDim, oldXOff, oldYOff, newXDim, newYDim, newXOff, newYOff;
	...
	GetCelRect(theView, lnum, cnum, IndexedProp(vId, actX), IndexedProp(vId, actY), IndexedProp(vId, actZ), (RRect*)IndexedPropAddr(vId, actNS));

	if (saveUnders != NULL)
		*unders = (Handle)Pseudo(SaveBits((RRect*)IndexedPropAddr(vId, actNS), VMAP | PMAP |CMAP));

	ScaleDrawCel(theView, lnum, cnum, (RRect*)IndexedPropAddr(vId, actNS), IndexedProp(vId, actPri), oldXDim);
...
global void AddToPic(List *cast)
{
	register Obj *him;
	ObjID it;
	RGrafPort *oldPort;
	...
	if (cast)
	{
		...
		for (it = FirstNode(cast); it; it = NextNode(it))
		{
			...
			if (-1 == castPri[castSize]) castPri[castSize] = CoordPri(castY[i]);
			++castSize;
		}
		...
		for (i = 0; i < castSize; i++)
		{
			...
			{
				GetCelRect(view, GetProperty(him, s_loop), GetProperty(him, s_cel), GetProperty(him, s_x), GetProperty(him, s_y), GetProperty(him, s_z), &r);
			...
			if (!(signal & ignrAct))
			{
				...
				if (top >= r.bottom) top = r.bottom - 1;
...
static void near ReDoStopped(Obj* cast[], byte castShow[], int size)
{
	...
	for (i = size - 1; i >= 0; i--)
	{
		...
		if (signal & NOUPDATE)
		{
			if (!(signal & HIDDEN))
			{
				...
				{
					if (underBits) UnloadBits((Handle) Native(underBits));
				}
				else
				{
			...
			if (signal & FORCEUPD) signal &= ~FORCEUPD;

			//logic must respect transition to UPDATING
			if (signal & STARTUPD) signal &= (~(STARTUPD | NOUPDATE));
		}
		else
		{
			//an updating actor that may want to stop
			if (signal & STOPUPD) signal = (signal & ~STOPUPD) |  NOUPDATE;
		}
		...
		if (signal & VIEWADDED)
		{
			...
			if ((scale & VM_SCALABLE))
				doScale(him, (Handle)view, (word)IndexedProp(him,actLoop),
							(word)IndexedProp(him, actCel), (Handle*)NULL, //(word)NULL KAWA WAS HERE
		...
		if (NOUPDATE & (signal = IndexedProp(him, actSignal)))
		{
			...
			if (!(signal & HIDDEN) )
			{
				...
				if((scale & VM_SCALABLE))
				{
					...
					IndexedProp(him, actUB) = Pseudo(SaveBits((RRect*)IndexedPropAddr(him, actNS), whichMaps));
#else
				if ((scale & VM_SCALABLE))
				{
					theViewNum = IndexedProp(him, actView);
					...
					IndexedProp(him, actUB) = Pseudo(SaveBits((RRect*)IndexedPropAddr(him, actNS), VMAP | PMAP | CMAP));
#endif
			}
		}
	}
	...
	for (i = 0; i < size; i++)
	{
		...
		if (signal & NOUPDATE && !(signal & HIDE))
		{
			...
			{
				DrawCel((Handle)view, IndexedProp(him, actLoop), IndexedProp(him, actCel), (RRect*)IndexedPropAddr(him, actNS), IndexedProp(him, actPri));
			}
			castShow[i] = 1;
...
global void ReAnimate(RRect* badRect)
{
	RGrafPort *oldPort;
	AniObj* dupPtr;
	Handle view;
