...
// > One type mismatch for a null pointer in KAnimate.
...
#endif
global KERNEL(Record);
global KERNEL(PlayBack);
...
global KERNEL(ResCheck)
{
	...
	switch (arg(1))
	{
		...
		case RES_AUDIO36:
		case RES_SYNC36:
			acc = (int)ResCheck36((byte)arg(1), (uint)arg(2), (byte)arg(3), (byte)arg(4), (byte)arg(5), (byte)arg(6));
			break;
		default:
			for (count = 2; count <= argCount; ++count)
			{
				acc = (int)ResCheck((byte)arg(1), (uint)arg(count));
				if (!acc) return;
		 	}
			break;
	}
...
global KERNEL(Load)
{
	...
	switch (arg(1))
	{
		...
		default:
			for (count = 2; count <= argCount; ++count)
		 		acc = Pseudo(ResLoad((byte)arg(1), (uint)arg(count)));
			break;
	}
}


global KERNEL(UnLoad)
{
	switch (arg(1))
	{
		case RES_MEM:
		...
		case RES_SYNC36:
			break;
		default:
			ResUnLoad((byte)arg(1), (uint)arg(2));
			break;
	}
}
...
//return (or set) picture validity
global KERNEL(PicNotValid)
{
	acc = noShowBits;
	if (argCount)
		noShowBits = arg(1);
...
//size rectangle for this text
global KERNEL(TextSize)
{
	word def = 0;
...
//animate this cast list
global KERNEL(Animate)
{
	RRect r, sumRect;
	...
	//If we are currently in a special effects cursor mode process accordingly
	...
	if (argCount == 2)
	{
		...
		Animate((List*)0, 0); //KAWA WAS HERE
}


global KERNEL(GetAngle)
{
	RPoint sp, dp;
...
global KERNEL(SinMult)
{
	acc = SinMult(arg(1), arg(2));
}


global KERNEL(CosMult)
{
	acc = CosMult(arg(1), arg(2));
}


global KERNEL(SinDiv)
{
	acc = SinDiv(arg(1), arg(2));
}


global KERNEL(CosDiv)
{
...
//textNum [at:] [color:] [back:] [noshow:]
global KERNEL(Display)
{
	word *lastArg;
	word mess;
	...
	RMove(r.right > 320 ? 320 - r.right : 0, r.bottom > 200 ? 200 - r.bottom: 0);
	MoveRect(&r, rThePort->pnLoc.h, rThePort->pnLoc.v); //reposition

	if (saveIt) acc = Pseudo(SaveBits(&r, VMAP));
...
global KERNEL(GetEvent)
{
	...
	{
		case RECORD:
			if(recordRoom != (signed)globalVar[g_curRoomNum]) //KAWA WAS HERE
			{
...
//draw a control object in current port
KERNEL(DrawControl)
{
	word *nRect;
...
//do proper hilighting of control
...
//call the kernel to edit an EditText item
global KERNEL(EditControl)
{
	acc = EditControl((Obj*)Native(arg(1)), (Obj*)Native(arg(2)));
}


//adjust the event record local coords
global KERNEL(GlobalToLocal)
{
	RGlobalToLocal((RPoint*)IndexedPropAddr((Obj*)Native(arg(1)), evY));
}


//adjust the event record global coords
global KERNEL(LocalToGlobal)
{
	RLocalToGlobal((RPoint*)IndexedPropAddr((Obj*)Native(arg(1)), evY));
}


//draw cel in current port at coords and pri
global KERNEL(DrawCel)
{
	RRect r;
	Handle view;
	uint loop, cel;
...
//add this view to current picture
global KERNEL(AddToPic)
{
...
//dispose of allocated memory associated with kList
global KERNEL(DisposeList)
{
	List *list;
	ObjID kNode;
...
global KERNEL(DisposeClone)
{
	DisposeClone((Obj*)Native(arg(1)));
}


global KERNEL(MemoryInfo)
{
	ulong size;
...
global KERNEL(IsObject)
{
	acc = IsObject((Obj*)Native(arg(1)));
}


global KERNEL(RespondsTo)
{
	acc = RespondsTo((Obj*)Native(arg(1)), arg(2));
}


global KERNEL(StrCmp)
{
	if (argCount == 2)
	...
	if (argCount >= 4)
	{
		...
		if (polylist = RNewPtr(sizeof(polygon) * (size + 1)))
		{
			for (it = FirstNode(theList), i = 0; it; i++, it = NextNode(it))
			{
				...
				polylist[i].type = (byte)GetProperty(him, s_type);
				polylist[i].info = 0;
			}
...
global KERNEL(Memory)
{
	switch (arg(1))
	{
		...
		case MDisposePtr:
			if (arg(2) & 1)
				PError(thisIP, pmsp, E_ODD_HEAP_RETURNED, arg(2), 0);
...
global KERNEL(FileIO)
{
	...
	switch (arg(1))
	{
		...
		case fileFPuts:
			...
			break;
		case fileSeek:
			acc = (int)lseek(arg(2), (long)arg(3), (byte)arg(4));
			break;
		case fileFindFirst:
...
//KAWA
		...
		case fileWriteByte:
			kawaByte = (char)arg(3);
			acc = write(arg(2), &kawaByte, 1);
			break;
		case fileReadWord:
			kawaWord = -1;
			cnt = read(arg(2), &kawaWord, 2);
			acc = kawaWord;
			break;
		case fileWriteWord:
			DoAlert(cpy);
...
KERNEL(Kawa)
{
	...
	{
		case 0: //KAWA_DOALERT
			//(Kawa 0 {text}) ;Displays text as a system alert.
			DoAlert((char*)arg(2));
			break;
		case 1: //KAWA_ZAWARUDO
			//(Kawa 1 from to) ;Inverts range of colors.
			for (i = arg(2); i < arg(3); i++)
			{
				sysPalette.gun[i].r = 255 - sysPalette.gun[i].r;
		...
		case 2: //KAWA_SETTITLECOLORS
			...
			winTitleBackColor = arg(2);
			winTitleTextColor = arg(3);
			break;
		case 3: //KAWA_ISDEBUG
