...
void pkImplode(int fd, byte far *src, unsigned length)
{
	...
	implode(pkReadFromMem, pkWriteToDisk, (char far*)*workBuf, (unsigned short far*)&type, (unsigned short far*)&dsize);
...
uint pkImplode2Mem(byte far *dst, byte far *src, unsigned length)
{
	...
	implode(pkReadFromMem, pkWriteToMem, (char far*)*workBuf, (unsigned short far*)&type, (unsigned short far*)&dsize);
