...
// > One signed/unsigned mismatch in KInitBresen, added cast to fix.
...
global KERNEL(InitBresen)
{
	...
	forever
	{
		...
		if (xAxis && tdx > tdy)
		{
			if (tdx && (int)abs(dy + incr) > tdy) //KAWA WAS HERE
			{
				if(!(--watchDog))
				{
					RAlert(E_BRESEN);
